...
address='192.168.136.143'   #测试服务器的ip地址
...
def get_net_info():
    ...
    for net_name,net_info in net_io.items():
        ...
        info = "{}: recv:{} MB,send:{} MB,pkg_recv:{},pkg_snd:{}\n".format(net_name,recv//1024//1024,send//1024//1024,pkg_recv,pkg_snd)
        #保存数据
...
def get_iops_info():
    ...
    for dname,dinfo in disk_io.items():
        ...
        info = "{}: read:{},write:{},read_bytes:{} MB,write_bytes:{} MB\n".format(dname,read_count,write_count,read_bytes//1024//1024,write_bytes//1024//1024)
        # 保存数据
...
def get_disk_info():
    ...
    for disk in psutil.disk_partitions():
        ...
        # 当前磁盘使用率和剩余空间G信息
        info = "Disk Mount:{},Disk Percent:{} %,Disk Free:{} GB\n".format(disk_mount, str(percent_disk), free_disk_size)
        # print(info)
        # 拼接多个磁盘的信息
        content = content + info
...
def main():
    ...
    while True:
        ...
        host = get_host_info()
        #获取网络数据
        # recv,send,pkg_recv,pkg_snd = get_net_info()
        net = get_net_info()
        recv, send, pkg_recv, pkg_snd = 0,0,0,0
        #获取磁盘使用空间
        disk = get_disk_info()
        #获取磁盘IO性能
        iops = get_iops_info()

        data = [tm,host,cpu_usage,cpu_load1,cpu_load5,cpu_load15,mem_total,mem_free,mem_usage,swap_total,swap_free,swap_percent,recv,send,pkg_recv,pkg_snd]
        data = map(str,data)
        senddata = ','.join(data)
        if len(senddata) == 0 :
            break
